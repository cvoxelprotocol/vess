...
export class Web3ModalService {
  ...
  setProvider(provider?: Web3Provider, account?: string, chainId?: number, web3Modal?: Web3Modal) {
  ...
  async connectWallet(): Promise<connectWalletProps> {
    ...
    if (!process.env.NEXT_PUBLIC_INFURA_KEY) throw new Error('Something went wrong..')
  ...
  async validateNetwork(): Promise<boolean> {
    ...
    if ((window as any).ethereum && (window as any).ethereum.networkVersion !== 1) {
      try {
