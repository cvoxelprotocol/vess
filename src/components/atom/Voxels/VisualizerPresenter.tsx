...
const VisualizerPresenter: FC<VisualizerPresenterProps> = ({
  ...
  const { displayVoxels } = useMultipleVoxelStyler(
    workCredentials.length > 0 ? workCredentials : [],
  )
  ...
  return (
    <>
      ...
      <OrbitControls
        ...
        minDistance={12}
      />
      <Plane receiveShadow rotation-x={-Math.PI / 2} position={[0, -3, 0]} args={[20, 20, 4, 4]}>
          ...
          (voxel, i) =>
            voxel && (
              <VoxelPresenter
                {...voxel}
                key={i}
                handleClick={isInitVoxels ? undefined : () => handleClickVox(voxel.id)}
